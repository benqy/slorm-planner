...
export class SlormancerItemService {
    ...
    public updateEquipableItemView(item: EquipableItem, defensivestatsMultiplier: number) {
        ...
        if (item.skillEnchantment !== null) {
            ...
            item.skillEnchantment.label = this.slormancerTemplateService.getReaperEnchantmentLabel(this.SKILL_ENCHANTMENT_LABEL, item.skillEnchantment.effect.value, min, max, skill === null ? '??' : skill.CH_NAME);
