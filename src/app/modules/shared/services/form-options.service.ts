...
export class FormOptionsService {
    ...
    private initLegendaryOptionsCache() {
        ...
        for (const legendary of legendaries) {
            const option: SelectOption<number> = { value: legendary.REF, label: legendary.CH_NAME }
    ...
    private initSkillBuffOptions() {
        this.ALL_SKILL_BUFF_OPTIONS_CACHE = {
            [HeroClass.Warrior]: this.slormancerDataService.getGameDataActiveSkills(HeroClass.Warrior)
                .map(skill => ({ value: skill.REF, label: skill.CH_NAME })),
    ...
    private initReaperOptions() {
        ...
        for (const reaper of reapers) {
            this.ALL_REAPER_OPTIONS_CACHE.p[HeroClass.Warrior].push({ value: reaper.REF, label: this.slormancerReaperService.getReaperName(reaper.CH_NAME, true, HeroClass.Warrior ) });
            this.ALL_REAPER_OPTIONS_CACHE.p[HeroClass.Huntress].push({ value: reaper.REF, label: this.slormancerReaperService.getReaperName(reaper.CH_NAME, true, HeroClass.Huntress ) });
            this.ALL_REAPER_OPTIONS_CACHE.p[HeroClass.Mage].push({ value: reaper.REF, label: this.slormancerReaperService.getReaperName(reaper.CH_NAME, true, HeroClass.Mage ) });
            this.ALL_REAPER_OPTIONS_CACHE.b[HeroClass.Warrior].push({ value: reaper.REF, label: this.slormancerReaperService.getReaperName(reaper.CH_NAME, false, HeroClass.Warrior ) });
            this.ALL_REAPER_OPTIONS_CACHE.b[HeroClass.Huntress].push({ value: reaper.REF, label: this.slormancerReaperService.getReaperName(reaper.CH_NAME, false, HeroClass.Huntress ) });
            this.ALL_REAPER_OPTIONS_CACHE.b[HeroClass.Mage].push({ value: reaper.REF, label: this.slormancerReaperService.getReaperName(reaper.CH_NAME, false, HeroClass.Mage ) });
        }
    }
